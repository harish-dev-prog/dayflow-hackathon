...
// Helper: today's date as YYYY-MM-DD (server local time)
function todayStr() {
  return new Date().toISOString().split("T")[0];
}

// Helper: N days ago as YYYY-MM-DD, used for the weekly view window
...
async function checkIn(req, res) {
  try {
    ...
    return res.status(201).json({
      ...
      attendance: { id: result.lastID, user_id: req.user.id, date, check_in: now, check_out: null, status: "present" },
    });
  } catch (err) {
...
async function checkOut(req, res) {
  try {
    ...
    const hoursWorked = (new Date(now) - new Date(existing.check_in)) / (1000 * 60 * 60);
    ...
    return res.status(200).json({
      ...
      attendance: { ...existing, check_out: now, status },
    });
  } catch (err) {
...
async function getMyAttendance(req, res) {
  try {
    ...
    return res.status(200).json({ range, attendance: rows });
  } catch (err) {
...
// GET /api/attendance/all?date=YYYY-MM-DD (admin - defaults to today)
async function getAllAttendance(req, res) {
  try {
    ...
    return res.status(200).json({ date, attendance: rows });
  } catch (err) {
...
// GET /api/attendance/:userId?range=daily|weekly (admin - view one employee)
...
// PATCH /api/attendance/:userId/status (admin - manually set status, e.g. absent or leave)
  ...
  try {
    ...
    if (!date || !status || !validStatuses.includes(status)) {
      return res.status(400).json({
        message: `date and a valid status (${validStatuses.join(", ")}) are required.`,
      });
    }
    ...
    if (existing) {
      ...
      // e.g. marking someone absent for a day they never checked in
      await db.run(
